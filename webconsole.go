				...
				if taskErr == nil {
					for _, task := range taskList {
						if strings.HasPrefix(requestPath, "/" + task["taskID"]) && serveFile == true {
							fmt.Println("Serve task file: " + arguments["taskroot"] + "/" + task["taskID"] + "/www" + requestPath[16:])
							http.ServeFile(theResponseWriter, theRequest, arguments["taskroot"] + "/" + task["taskID"] + "/www" + requestPath[16:])
							serveFile = false
						}
					}
				}
